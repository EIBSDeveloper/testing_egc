...
<script>
    ...
    var goldBasedIncentives = ['presale', 'megabonus'];
    ...
    function renderIncentiveUI(data) {
                        ...
                        <h2 class="accordion-header">
                            <button class="accordion-button collapsed d-flex justify-content-between align-items-center ${borderClass}" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_${index}">
                                <div class="d-flex align-items-center gap-3 w-100 flex-wrap flex-md-nowrap">
                                    ...
                                    <button class="btn btn-sm btn-outline-info me-2" onclick="event.stopPropagation(); downloadSinglePDF(${index})" title="Download PDF"><i class="mdi mdi-download"></i> PDF</button>
                                </div>
                            </button>
                        </h2>
        ...
        html = `<div class="text-end mb-3"><button class="btn btn-primary" onclick="downloadAllPDF()"><i class="mdi mdi-download"></i> Download All Report (PDF)</button></div>${html}`;
  ...
  function renderIncentiveRows(staff) {
      ...
      orderedKeys.forEach(key => {
          ...
          // Check if originalValue is a string (non-numeric award like trips, iPhone)
          if (typeof originalValue === 'string' && isNaN(originalValue)) {
              // This is a non-monetary award (Trip, iPhone, etc.)
              let awardClass = '';
              if (originalValue.includes('Trip')) awardClass = 'trip';
              if (originalValue.includes('Iphone') || originalValue.includes('iPhone')) awardClass = 'iphone';
              awardDisplay = `<span class="award-text ${awardClass}"><i class="mdi mdi-trophy-variant fs-5"></i> ${originalValue}</span>`;
          } else if (isGold) {
              // Gold award
              let goldValue = Number(originalValue) || 0;
              awardDisplay = `<i class="mdi mdi-gold fs-4" style="color:#FFD700;"></i> ${goldValue} gm Gold`;
          } else {
              // Monetary award
              let moneyValue = Number(originalValue) || 0;
              awardDisplay = `<i class="mdi mdi-currency-inr fs-3"></i> ${moneyValue.toLocaleString()}`;
          }
